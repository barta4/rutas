    ...
    const handleStartOrder = async (item) => {
        ...
        try {
            ...
            try {
                ...
                // We don't block for long, but we must ensure network packet is sent before backgrounding
                await api.post(`/driver/orders/${item.id}/start`, {
                    lat: loc?.latitude,
                    lng: loc?.longitude
                }).catch(err => console.error("API Start Warning:", err.message));
                // We catch individual error here so it doesn't stop the map from opening
